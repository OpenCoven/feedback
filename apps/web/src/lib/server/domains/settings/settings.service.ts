  ...
  try {
    ...
    if (updated.ssoOidc) {
      ...
      if (discoveryUrlChanged && isHttps && updated.ssoOidc.discoveryUrl) {
        const { validateOidcDiscoveryForServerRuntime } =
          await import('@/lib/server/auth/oidc-discovery-validation')
        const validation = await validateOidcDiscoveryForServerRuntime(updated.ssoOidc.discoveryUrl)
        if (!validation.ok) {
          throw new ValidationError(
            'INVALID_SSO_CONFIG',
